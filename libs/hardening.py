...
class WXOR():
  ...
  def harden_bf(self, bf):
    ...
    assert bf_len == len(wxor_bf)

    str_idx = 0
    end_idx = self.win_size   

    for ite in xrange(bf_len):
 
      if (str_idx + self.win_size) > bf_len:
         break

      first_window = wxor_bf[str_idx:end_idx]

      if end_idx > (bf_len-1):
         
        bit_str = wxor_bf[str_idx+1:].to01() + str(1 if wxor_bf[0] else 0)
        
        end_window = bitarray.bitarray(bit_str)

      else:
        end_window = wxor_bf[str_idx+1:end_idx+1]  

      
      xor_arr = first_window ^ end_window

      for idx in xrange(self.win_size):
...
class RESAMPLE():
  ...
  def harden_bf(self, bf):
    ...
    # Initialise bitarray for a new Bloom filter
    #
    resamp_bf = bitarray.bitarray(bf_len)
    resamp_bf.setall(0)
    ...
    for i in range(bf_len):
        ...
        bit_val = 1 if (bf[first_bit] ^ bf[second_bit]) else 0
...
#      print 'extra q-gram set 3:', sorted(extra_q_gram_set3)
  ...
  assert bf_wxor_hardened2.count() != bf_wxor_hardened1.count()
